...
class V0_7 : IChessBot
{
    ...
    public Move Think(Board board, Timer timer)
    {
        ...
        result.BestMoves.Add(GetOrderdMoves(board, false)[0]);
    ...
    private SearchResult NegaMax(Board board, int depth, int alpha = MIN_SCORE, int beta = MAX_SCORE, int color = 1, int ply = 0)
    {
        ...
        var moves = GetOrderdMoves(board, quiesce);
    ...
    private Move[] GetOrderdMoves(Board board, bool quiesce)
    {
        return board.GetLegalMoves(quiesce).OrderByDescending(move => move.IsCapture ? (int)move.CapturePieceType * 100 + (int)move.MovePieceType : 0).ToArray();
    }
