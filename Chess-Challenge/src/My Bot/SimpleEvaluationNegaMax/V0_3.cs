...
class V0_3 : IChessBot
{
    ...
    public Move Think(Board board, Timer timer)
    {
        ...
        Move[] moves = { GetOrderdMoves(board)[0] };

        int depth = 0;
        while (_timer.MillisecondsElapsedThisTurn < _maxTime && depth < 50)
        {
            moves = NegaMax(board, depth);
            depth++;
        }
    ...
    private Move[] NegaMax(Board board, int depth)
    {
        ...
        {
            board.MakeMove(move);
            int value = -NegaMax(board, depth - 1, MIN_SCORE, MAX_SCORE, -1);
            board.UndoMove(move);
    ...
    private int NegaMax(Board board, int depth, int alpha, int beta, int color)
    {
        ...
        if (depth <= 0 || moves.Count() == 0) return color * Evaluate(board, depth);
        int value = MIN_SCORE;
